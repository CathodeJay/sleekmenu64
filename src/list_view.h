...
/* Adjust an existing window so `selected` is inside it, moving as little as
   ...
   still moves a row at a time but always starts on a row boundary. The grid
   used to snap to whole pages instead, which meant one step down replaced all
   twelve covers on screen -- twelve reads off the card for a move that
   actually changed four tiles. */
