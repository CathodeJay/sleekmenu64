...
void sm_genre_build_tabs(const sm_catalog_t *catalog, sm_genre_tabs_t *out) {
    ...
    /* One pass, counting every genre on the card. This used to stop admitting
       new ones once eight slots were full, which made the tabs depend on the
       order the catalog happened to be in: whichever genres appeared first
       won, the sort below only reordered those, and the rest of the library
       was unreachable. */
