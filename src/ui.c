...
/* Point the slots at the current window, carrying over any sprite already
   ...
   files, so it can run the same frame the scroll happened. Scrolling one row
   used to free all twelve sprites and read all twelve back, eight of which
...
/* The next distinct value in alphabetical order, or NULL for "Any".
   ...
   This used to step through the catalog in storage order and only reach
   "Any" by falling off one end of it, which on a card with two hundred
   publishers meant two hundred presses to clear the filter -- reported, quite
   reasonably, as not being able to clear it at all. Sorted order also means
   the list reads the way a list should. */
...
static const char *footer_hint(const sm_catalog_t *c, const sm_ui_t *ui) {
    static char line[64];
    sm_game_t game;
    /* Forty-eight characters is what the safe area holds at six pixels each.
       The old line was fifty-five and lost its tail, which is where the
       favourite key would have been. C-left and C-right walk the tab strip,
       which is "all", favourites, then the genres -- so the word is TABS
       rather than GENRE, and starring a game is STAR rather than FAV, which
       otherwise named two different things in one line. */
...
/* A folder tile. Drawing the first game inside it -- which is what a folder
   item stores, and what this used to render -- made a folder look like a game
   that had already been opened, and gave four different folders four
   different, wrong names. */
...
/* A folder, drawn once into a cover-shaped buffer.
   ...
   A folder used to get the same blank rectangle as a cover that has not
   streamed off the card yet, so the two were indistinguishable -- which on a
   shelf that is constantly loading meant you could not tell a folder from a
   picture that was about to appear.
...
void ui_draw(surface_t *s, const sm_layout_t *l, const sm_catalog_t *c, const sm_ui_t *ui) {
    ...
    /* --- the library, dense --------------------------------------------- */
    {
        ...
        /* Both shortlists are flat, so "flat" is no longer the same question
           as "favourites" -- asking the old one here put FAVOURITES over the
           history list. */
        if (ui->genre_tab == SM_STRIP_HISTORY)
