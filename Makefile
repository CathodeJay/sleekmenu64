...
.PHONY: all help test catalog card metadata coverdb card-art \
...
help:
	...
	@echo "  make catalog    Build a catalog from examples/sample_metadata.json"
...
catalog:
	mkdir -p $(BUILD_DIR)
	$(PYTHON) tools/build_catalog.py examples/sample_metadata.json \
...
# carry; docs/sd-forensics/ explains how the layout was established.
registry:
	@if [ -z "$(FILE)" ]; then \
...
# screen without it. Box art does not, and used to -- 915 sprites and a
# name-matching table in C, 10 MB of ROM, and a build that could not run at
# all without one particular art collection on disk. Covers come off the card
# now, matched once on a desktop against data/coverdb.csv. The font itself
# is Spleen 5x8 (BSD-2-Clause), vendored as its BDF and rasterised here.
font-dfs:
