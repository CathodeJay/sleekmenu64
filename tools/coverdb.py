...
The alternative -- every user re-deriving the same facts on their own
machine by comparing file names against a database -- puts a fuzzy string
match in the path of every card, and a library named even slightly
differently gets half its metadata and no explanation for the rest.

The Dreamcast scene settled this long ago. openMenu keys its metadata
