...
Everything a card needs used to be five tools driven by a Makefile, one of
which shelled out to a MIPS cross-compiler to resize a PNG. That is fine for
whoever wrote it and hostile to everyone else, which is why the Dreamcast scene
long ago settled on a single card manager and never looked back.

This does the same things in order, in this process:
